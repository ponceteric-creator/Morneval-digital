...
const TIE_BREAK = ["population", "institutions", "external_markets"];
...
export function getDemandPriorityGroups(religionArcane, militaryMercantile) {
  ...
  if (religionArcane !== 0) preferred.add("institutions");
  if (militaryMercantile < 0) preferred.add("population");
  if (militaryMercantile > 0) preferred.add("external_markets");
...
export function allocateDemand(supply, demand, priorityGroups) {
  ...
  for (const group of priorityGroups) {
    ...
    for (const category of orderedGroup) {
      ...
      for (let i = 0; i < amount; i++) serviceSequence.push(category);
...
function getSectorConfig(sectorId) {
  ...
  if (!sectorConfig) throw new Error(`No prototype configuration for sector: ${sectorId}`);
  return sectorConfig;
}
...
export function resolveSectorEconomy(state, sectorId) {
  ...
  for (let i = 0; i < servedStakes.length; i++) {
    const stake = servedStakes[i];
    const category = demand.serviceSequence[i];
    const wealth = BALANCE_CONFIG.wealthPerDemand[category] ?? 0;
  ...
  for (const player of state.players) {
    player.wealthGeneratedThisGeneration = player.productionStakes.reduce(
      (sum, stake) => sum + stake.wealthProducedThisGeneration,
      0,
    );
  }
  ...
  return {
    ...
    servedStakes: servedStakes.map(stake => ({
      ...
      demandCategory: stake.servedDemandCategory,
...
export function resolveAllSectorEconomies(state) {
  ...
  for (const sector of state.productionSectors) {
    reports.push(resolveSectorEconomy(state, sector.id));
  }

  const finalWealth = state.players.map(player => ({
    playerId: player.id,
    familyName: player.familyName,
    wealthGenerated: player.wealthGeneratedThisGeneration,
  }));
  for (const report of reports) report.familyWealth = finalWealth.map(item => ({ ...item }));
  return reports;
}
