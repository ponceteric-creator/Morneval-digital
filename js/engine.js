...
export function createGame(familyNames) {
    ...
    return {
        ...
        productionSectors: [
            {
                ...
                name: "Food",
                tier: 1,
                totalDemandThisGeneration: 1,
                demandThisGeneration: { population: 1, institutions: 0, external_markets: 0 },
            },
            {
                id: "textiles",
                name: "Textiles",
        ...
        lands: CONFIG.initialLands.map((land) => ({
            ...land,
            capacityModifier: 0,
...
export function placeProductionStake(state, playerId, sectorId) {
    ...
    const allSectorStakes = state.players.flatMap((p) => p.productionStakes.filter((s) => s.sectorId === sectorId));
    if (allSectorStakes.length >= sector.tier) {
        throw new Error("No available Stake slot in this Production Sector.");
    }
...
export function developSector(state, playerId, sectorId) {
    ...
    if (nextTier > 3)
        throw new Error("Prototype supports only three tiers.");
    const config = CONFIG.sectorDevelopment[nextTier];
    if (player.influence < config.influenceCost) {
        throw new Error("Not enough Influence to develop Sector.");
    }
...
export function erodeInfluence(state) {
    for (const player of state.players) {
        player.influence = Math.max(0, player.influence - CONFIG.influenceErosion);
    }
}
