...
let message = `v${VERSION} ready. Wealth is shown as an explicit Population / Institution / External breakdown.`;
...
function clearEconomicState() {
  ...
  for (const land of game.lands) land.usedCapacityThisGeneration = 0;
}
...
function wealthBreakdown(playerId) {
  ...
  for (const report of reports) {
    for (const stake of report.servedStakes ?? []) {
      ...
      const category = stake.demandCategory;
      const amount = Number(stake.wealthGenerated) || 0;
      if (category && category in result) result[category] += amount;
      result.total += amount;
    }
  }
  return result;
...
function institutionPanel() {
  ...
  const warning = status.overCap
    ? `<div class="warning"><b>Over population cap:</b> Institution levels total ${status.total}, Population is ${status.cap}. Further development is blocked.</div>`
  ...
  return `<section class="panel institution-panel">
    ...
    <div class="notice">Only Institution levels are modeled here. Total Institution levels may not be increased above Population.</div>
    ${warning}
    ...
    <div class="prototype-rules"><b>Auto Institution demand:</b> total Institution levels = ${totalInstitutionLevels(game)}.</div>
  </section>`;
}
...
function render() {
  ...
  root.innerHTML = `<main class="app-shell">
    <header class="topbar">
      <div class="brand"><h1>MORNEVAL</h1><p>Generational balance sandbox · Engine v${VERSION}</p></div>
    ...
    <div class="notice"><b>v${VERSION}:</b> 1 Production Stake = 1 supply unit = 1 need satisfied, subject to raw-resource capacity. Wealth per served unit: <b>Population ${BALANCE_CONFIG.wealthPerDemand.population} / Institutions ${BALANCE_CONFIG.wealthPerDemand.institutions} / External ${BALANCE_CONFIG.wealthPerDemand.external_markets}</b>.</div>
    ...
    <section class="panel simulation-panel">
      ...
      <div class="prototype-rules"><b>Scaling:</b> Population demand uses Population; Institution demand uses total Institution levels (${instStatus.total}); External demand uses Renown (${game.city.renown}).</div>
    </section>
    ...
    <section class="report-list">${reports.length ? reports.map(renderReport).join("") : '<div class="panel empty">No economy preview/resolution yet. Wealth is cleared whenever an economic input changes.</div>'}</section>
    ...
    <p class="footer-note">Renown is manually adjustable because its final growth rules are not yet defined. Balance-parameter editing is temporarily disabled in this stability build.</p>
  </main>`;
  bindEvents();
}

function renderFamily(player) {
  const breakdown = wealthBreakdown(player.id);
  return `<article class="panel family-card"><h3>${player.familyName}</h3><div class="family-stats"><div class="family-stat"><b>${player.prestige}</b><span>Prestige</span></div><div class="family-stat"><b>${player.influence}</b><span>Influence</span></div><div class="family-stat"><b>${player.wealthGeneratedThisGeneration}</b><span>Wealth</span></div></div><div class="land-score-preview">Wealth breakdown: Population <b>${breakdown.population}</b> · Institutions <b>${breakdown.institutions}</b> · External <b>${breakdown.external_markets}</b> · Report total <b>${breakdown.total}</b><br>Productive-land Prestige if generation ended now: <b>+${awardForPlayer(player.id)}</b></div></article>`;
}
...
function renderSector(sector) {
  ...
  return `<article class="panel sector-card">
    ...
    <button class="sector-resolve" data-resolve-sector="${sector.id}">Preview ${sector.name} only</button>
  </article>`;
}

function renderStake(stake) {
  return `<div class="stake ${stake.age}"><div class="stake-main"><b>${playerName(stake.ownerId)}</b><div class="meta">Placed #${stake.placementOrder}</div></div><div class="stake-edit"><select data-stake-owner="${stake.id}">${playerOptions(stake.ownerId)}</select><select data-stake-age="${stake.id}">${ageOptions(stake.age)}</select><button class="danger-small" data-remove-stake="${stake.id}">×</button></div></div>`;
}
...
function bindEvents() {
  ...
  document.querySelector("#advance-generation")?.addEventListener("click", () => {
    ...
    previewLandAwards = [];
    message = `Generation ${result.generation} resolved. Wealth: ${game.players.map(p => `${p.familyName} ${p.wealthGeneratedThisGeneration}`).join(", ")}.`;
    render();
  });
  ...
  document.querySelector("#advance-three")?.addEventListener("click", () => {
    ...
    previewLandAwards = [];
    message = `Generations ${start}–${game.generation - 1} resolved with no intervening player actions. Family cards show the last resolved generation.`;
    render();
  });
  ...
  for (const input of document.querySelectorAll("[data-city]")) {
    input.addEventListener("change", () => {
      syncInputs();
      invalidate("City value changed; demand recalculated and previous Wealth cleared.");
    });
  }

  for (const input of document.querySelectorAll("[data-institution-level]")) {
    input.addEventListener("change", () => safe(() => setInstitutionLevel(game, input.dataset.institutionLevel, input.value), "Institution level changed; previous Wealth cleared."));
  }
  ...
  for (const button of document.querySelectorAll("[data-resolve-sector]")) {
    button.addEventListener("click", () => {
      ...
      reports = [resolveSectorEconomy(game, id)];
      normalizeActualResourceUsage(game, reports);
      message = `${reports[0].sectorName} sector-only preview resolved. Family Wealth shown is for this Sector only.`;
      render();
    });
  }
  ...
  for (const select of document.querySelectorAll("[data-axis]")) {
    select.addEventListener("change", () => {
      ...
      invalidate("City Inclination changed; previous Wealth cleared.");
    });
  }
  ...
  for (const select of document.querySelectorAll("[data-tier-sector]")) {
    select.addEventListener("change", () => {
      ...
      invalidate("Sector tier changed; previous Wealth cleared.");
    });
  }
  ...
  for (const button of document.querySelectorAll("[data-add-stake]")) {
    button.addEventListener("click", () => {
      ...
      safe(() => addProductionStakeForTesting(game, owner, id, age), `Added ${age} Stake; previous Wealth cleared.`);
    });
  }

  for (const button of document.querySelectorAll("[data-remove-stake]")) {
    button.addEventListener("click", () => safe(() => removeProductionStake(game, button.dataset.removeStake), "Stake removed; previous Wealth cleared."));
  }

  for (const select of document.querySelectorAll("[data-stake-owner]")) {
    select.addEventListener("change", () => safe(() => setProductionStakeOwner(game, select.dataset.stakeOwner, select.value), "Stake owner changed; previous Wealth cleared."));
  }

  for (const select of document.querySelectorAll("[data-stake-age]")) {
    select.addEventListener("change", () => safe(() => setProductionStakeAge(game, select.dataset.stakeAge, select.value), "Stake age changed; previous Wealth cleared."));
  }

  for (const select of document.querySelectorAll("[data-land-owner]")) {
    select.addEventListener("change", () => safe(() => setResourceStakeOwner(game, select.dataset.landOwner, select.value || null), "Raw-resource owner changed; previous Wealth cleared."));
  }

  for (const input of document.querySelectorAll("[data-land-base]")) {
    input.addEventListener("change", () => safe(() => setLandBaseCapacity(game, input.dataset.landBase, input.value), "Land capacity changed; previous Wealth cleared."));
  }

  for (const input of document.querySelectorAll("[data-land-modifier]")) {
    input.addEventListener("change", () => safe(() => setLandCapacityModifier(game, input.dataset.landModifier, input.value), "Land modifier changed; previous Wealth cleared."));
  }
}
